...
char* test_constrator() {
    // creation
    GISTrainer t;
    /* currently no way to test this, disabled
    ...
    BOOST_CHECK_THROW(t.load_training_data("data/me_gis_train.ev", "no model"),
            runtime_error);
    BOOST_CHECK_THROW(t.load_training_data("no events", "data/me_model_txt"),
            runtime_error);
            */
